    ...
    switch (tabla) {
        ...
        default:
            return res.status(400).json({
                ok: false, mensale: 'Los criterios búsqueda son sólo: Usuarios, Mádicos y Hospitales.', error: { message: 'El criterio de búsqueda introducido no es válido.' }
            });
    }
...
// Búsqueda general de resultados
...
app.get('/todo/:busqueda', (req, res, next) => {
    ...
    Promise.all([buscarHospitales(busqueda, regex), buscarMedicos(busqueda, regex), (buscarUsuarios(busqueda, regex))]).then(respuestas => {
        res.status(200).json({
            ok: true, hospitales: respuestas[0], medicos: respuestas[1], usuarios: respuestas[2]
        });
    });
});
